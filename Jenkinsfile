    ...
    stage('Deploy Backend to Tomcat') {
      steps {
        sh '''
          ...
          rm -f "${TOMCAT_WEBAPPS}/2300031063-backend.war" || true
          rm -rf "${TOMCAT_WEBAPPS}/2300031063-backend" || true

          WAR_FILE=$(ls backend-springbootapp/target/*.war | head -n 1)
          if [ -z "$WAR_FILE" ]; then
            echo "ERROR: No WAR found in backend-springbootapp/target/"
            exit 1
          fi
